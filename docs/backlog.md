...
| B-013 | Deploy the frontier reconciliation gate | Prevent stale artifact re-promotion in the installed runtime | Green source review, static allowlist sync without `state/`, installed tests, primary-owned read-only frontier audit, explicit legacy reconciliation | implemented_pending_review | supervise-repo-loop source / exact primary Coordinator | Review and merge the isolated source change first; do not infer or initialize live frontiers from the repair task |
...
| ID | Purpose | Effect | Requirements | State | Owner | Next move |
|---|---|---|---|---|---|---|
| B-007 | Reconcile ClipPipeGen to its certified current gate | Remove chronology drift without selecting an artifact from this static backlog | Current Git/authority high water, exact Supervisor/human disposition lineage, FrontierCertificate | frontier_reconciliation_required | exact primary Coordinator after B-013 | Run the read-only frontier audit; do not present any historical Clip artifact for review until reconciliation applies |
| B-008 | Reconcile FastFictionFactory to its certified current gate | Prevent benchmark/Densou or another historical candidate from replacing the actual current disposition | Current Git/authority high water, exact Supervisor result identity, epoch CAS | frontier_reconciliation_required | exact primary Coordinator after B-013 | Audit and apply only an exact current result; delivery or an old static row is insufficient |
| B-009 | Reconcile NLMYTGen to its certified current gate | Keep runtime/evidence work bound to the actual accepted frontier | Current Git/authority high water, exact runtime and Supervisor result lineage, epoch CAS | frontier_reconciliation_required | exact primary Coordinator after B-013 | Audit without probing or dispatch; reconcile the exact current disposition before ordinary work |
| B-010 | Reconcile Residual Atlas to its certified current gate | Prevent an older preview or successor request from outranking later human acceptance | Current Git/authority high water, human/Supervisor lineage, FrontierCertificate | frontier_reconciliation_required | exact primary Coordinator after B-013 | Audit and preserve human precedence; do not request a generic successor from legacy state |
